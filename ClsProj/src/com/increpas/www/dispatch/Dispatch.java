...
public class Dispatch extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		// 이 함수는 서버가 기동된 이후
		// 요청내용이 .cls로 끝나는 요청이 왔을 경우 처음 딱 한번만 실행 되는 함수이다.
		// 최초로 이 서블릿이 시작되면 준비된 properties 파일을 읽어서
		// 이것을 이용해서 실제 실행가능한 클래스로 요청내용과 클래스객체를 맵핑을 해 놓아야 한다.
		...
		try {
			// 서버가 호스팅 될 경우 해당파일이 컴파일 되었을 때 주소를 알수 없으므로
			// 해당 자바 소스의 주소를 반환받는 변수를 만든다.
			String path = this.getClass().getResource("").getPath();
			// getResource("") 안에 / 또는 increpas를 기재하여 확인해보기.
		...
		// 키값을 알아냈으니 이제 실제 실행해야 할 클래스의 경로를 알아 낼 수 있다.
